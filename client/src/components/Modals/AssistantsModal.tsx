...
export default function AssisantsModal({
  show,
  onHide,
  assistantUpdate,
  saveAssistant,
      ...
      <Modal animationType="slide" transparent={true} visible={show}>
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            <Input
              value={data.name}
              handleChange={text => newAssistant(text, 'name')}
              style={{width: 300, marginBottom: 15}}
              label="Name"
            />
            <Input
              value={data.email}
              handleChange={text => newAssistant(text, 'email')}
              style={{width: 300, marginBottom: 15}}
              label="Email"
            />
            <Input
              value={data.phone}
              handleChange={text => newAssistant(text, 'phone')}
